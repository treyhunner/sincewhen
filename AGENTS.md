...
- **Silence is an answer, and plenty of names get it.** 9259 names are asked about and 6571 published. 1442 come back from `dating.py` with the sources contradicting each other; 1246 rest on a 3.x inventory diff nothing corroborates, which is `_publish`; 168 are in a module `UNREADABLE` names; and 10024 more never reach the question because the newest Python no longer documents them. The class tier is still the harder half, 4193 of the questions and 2653 of the answers, but reading the class bodies in `Lib/*.py` closed most of the gap: 591 class members are dated by the source outright and 510 fewer come back unanswered.
