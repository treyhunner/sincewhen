...
- **The builds outlast the table, though, and asking one directly is the fastest tool here.**
  They sit in `.cache/pythons/`, `binary()` finds one, and `just ask-pythons` puts a question the table cannot hold: not "does this name resolve" but "will this release parse this", which is the only way to ask about syntax at all.
  Both entries this repo has for a `__future__`-gated feature were settled that way, and so was the yield expression entry, in seconds each.
  Three things to know before believing an answer.
...
- **A third, of the same shape, that the removal work exposed.**
  `date_symbol` and `removal_of` both take a bare name and have to guess what kind of thing it is, and the dataset already knows: the matcher field says so.
  Where the two disagree the guess wins and the entry is wrong.
  `cmp` is the case: it is a builtin from 1.0 and a module from 0.9 to 1.5, the module rule means the module answers, and a `builtins` entry then reads as "0.9 or earlier" from `lib/cmp.py`.
  Both of its evidence tables are `manual` for that reason.
  `repr` is the same shape and benign: it is a builtin from 1.0 and `Lib/repr.py` from 1.0 until 3.0 renamed it `reprlib`, so the module answers and the two dates coincide, which is why that entry carries ordinary `source` evidence with a note rather than a `manual` one.
  Benign is not the same as safe, and it is why the builtin was missing from the dataset until somebody noticed: `propose.py` drafts a `repr` module entry for the name, so nothing offers to write the builtin one.
  The fix is to pass the kind in, which touches `verify_dataset`, `memberindex` and `propose`, so it wants its own pass like the two below.
...
- **A `__future__` gate is not availability.**
  A feature a release will only compile after `from __future__ import ...` is dated to the release that compiles it on its own, and the import keeps the older date under its own entry.
  `print()` is 3.0 against a 2.6 `from __future__ import print_function`, and the same reading makes the `with` statement 2.6 rather than PEP 343's 2.5, and a generator function 2.3 rather than PEP 255's 2.2.
  What decides it is that `minimum_version()` is a floor for the code as written, and `with open(p) as f:` on a line by itself is a SyntaxError in 2.5.
  The removal axis already reads it this way: `<>` is removed in 3.0 and not in 3.10, though `from __future__ import barry_as_FLUFL` re-enables it in every 3.x grammar through 3.9.
  Neither a PEP header nor the grammar can settle one of these, which is why all three entries are `manual`.
  A PEP headers the release its future import began working, and the grammar has the rule a release before its compiler will accept one: the 2.5 grammar has `with_stmt` and the 2.2 grammar has `'yield'`.
  That is "the grammar dates the parser and not the language" on the addition side, and it is the built interpreters that answer instead: one asks each release for the feature alone and then under its import.
  A syntax probe would make this mechanical rather than manual, which is what issue #11 is for.
  The pair over-reports for a file that carries the import, since a 2.5 file with both lines in it reports 2.6, and that is the same limit `print()`'s own note already records.
  It is the right way round: the alternative under-reports for every file that does not, which is nearly all of them, and reporting a feature as older than it is is the error this project most wants to avoid.
