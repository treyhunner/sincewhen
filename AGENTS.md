...
- **Silence is an answer, and most names get it.** 9259 names are asked about and 6018 published. 1963 come back from `dating.py` with the sources contradicting each other; 1277 rest on a 3.x inventory diff nothing corroborates, which is `_publish`; 168 are in a module `UNREADABLE` names; and 10024 more never reach the question because the newest Python no longer documents them. The class tier is the harder half by a distance: 4193 of the questions and 2097 of the answers, so half of what is asked about a class member goes unanswered against a fifth of what is asked about a module member.
