...
Handing the question to `dating.py` took the members that disagree with a dataset entry from 6 to 1, and that one is a pipeline bug rather than a ranking mistake.
...
- **Silence is an answer, and most names get it.** 5064 names are asked about and 3723 published. 422 come back from `dating.py` with the sources contradicting each other; 907 rest on a 3.x inventory diff nothing corroborates, which is `_publish`; 165 are in a module `UNREADABLE` names; and 8877 more never reach the question because the newest Python no longer documents them.
...
- **The check that earns it is the dataset.** 1245 members have both an entry and an index answer, and all but `importlib.import_module` agree. `test_the_index_agrees_with_the_dataset_where_both_speak` is that check and its exception list should stay at one: any other disagreement means the index is reading the pipeline differently from the way the dataset was built.
...
`tests/test_members.py` does the same for the member index, and its load-bearing test is `test_the_index_agrees_with_the_dataset_where_both_speak`: 1245 members have both an entry and an index answer, and every one of them has to match bar a single named exception.
