...
- **The builds outlast the table, though, and asking one directly is the fastest tool here.** They sit in `.cache/pythons/`, `binary()` finds one, and `just ask-pythons` puts a question the table cannot hold: not "does this name resolve" but "will this release parse this", which is the only way to ask about syntax at all. Both entries this repo has for a `__future__`-gated feature were settled that way, and so was the yield expression entry, in seconds each. Three things to know before believing an answer. The oldest six releases are built for i386 and will not exec on an x86-64 host with no 32-bit loader, which fails with `No such file or directory` and reads exactly like a missing build, so those are reported as `unasked` rather than as a refusal and are asked inside the image that built them. A `yes` can be the wrong `yes`: 1.5 through 2.2 compile a bare `yield` because `yield` is an ordinary name there and the line does nothing. And nothing it prints is written down anywhere, deliberately, because evidence needs the control snippet that issue #11 is about.
