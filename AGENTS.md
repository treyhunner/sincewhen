...
- A marker attaches to *every* signature above it, because a directive may carry several and one description, which the text build renders as adjacent signature lines with nothing between them. Keeping only the last of a group cost 215 markers and cost them silently, since a name with no marker is dated by whatever else spoke rather than raising: `assertNotEndsWith` was dated by the docs and `assertEndsWith` by the release that indexed it, four of the eight assertions 3.14 added were `inventory-only` for that reason, and `os.spawnl` and its four siblings read as "2.2 or earlier" against a 1.6 marker sitting under all eight of them. What separates a group from a described sibling is prose: `annotations_in` tracks whether the last thing it read was a signature, and only then does the next one join rather than replace.
  A grouped marker that quotes one of the group's own names is about that one. `typing.Never` and `typing.NoReturn` share a directive and carry a marker each, "Added in version 3.6.2: Added `NoReturn`" and "Added in version 3.11: Added `Never`", and reading the group as a whole gave `Never` the 3.6.2 and lost it from the index outright, the interpreters having proved it is not in 3.6. `mmap.MAP_STACK` is the same shape at scale, one directive listing 21 flags and four markers naming which arrived when. This applies only to a group: a marker under a lone signature dates that signature whatever its prose mentions, which is the reading every existing entry rests on.
...
- **Silence is an answer, and most names get it.** 9259 names are asked about and 6019 published. 1963 come back from `dating.py` with the sources contradicting each other; 1277 rest on a 3.x inventory diff nothing corroborates, which is `_publish`; 168 are in a module `UNREADABLE` names; and 10024 more never reach the question because the newest Python no longer documents them. The class tier is the harder half by a distance: 4193 of the questions and 2098 of the answers, so half of what is asked about a class member goes unanswered against a fifth of what is asked about a module member.
...
`tests/test_members.py` does the same for the member index, and its load-bearing test is `test_the_index_agrees_with_the_dataset_where_both_speak`: 1277 members have both an entry and an index answer, and every one of them has to match.
